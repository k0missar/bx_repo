...
var_dump('BOOTSTRAP_V4');
// МУСОРНЫЙ КОД ДЛЯ ШАБЛОНА
...
?>
<div class="row bx-<?=$arParams['TEMPLATE_THEME'] // МУСОРНЫЙ ПАРАМЕТР ТЕМЫ ?>">
	<div class='<?=($isSidebar ? 'col-md-9 col-sm-8' : 'col') // МУСОРНЫЙ ПАРАМЕТР САЙДБАРА ?>'>
		<? // Вывод сравнения
		...
		if ($elementId > 0)
		{
			...
			// МУСОРНЫЙ УСЛОЖНЕННЫЙ КОД ДЛЯ ВЫВОДА СВЯЗАННЫХ ТОВАРОВ (ПОПУЛЯРНОЕ, С ЭТИМ ТОВАРОМ ПОКУПАЮТ И Т.П.)
			...
			if (!empty($recommendedData))
			{
				...
				// ПОКАЗАТЬ БЛОК ПОПУЛЯРНОЕ DETAIL_SHOW_POPULAR
				...
				// ПОКАЗАТЬ БЛОК ПРОСМАТРИВАЛИ DETAIL_SHOW_VIEWED
				if (
