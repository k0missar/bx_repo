...
<div class="row">
	<div class='col'>
		...
		// Параметры компонента детальной товаров
		...
		// Все что касается кода ниже - блок согласия - берется из arParams и прокидывается дальше
		// Включает механизм пользовательского согласия
		...
		// Отмечен ли чек бокс согласия по умолчания или нет
		if (isset($arParams['USER_CONSENT_IS_CHECKED']))
		{
			$componentElementParams['USER_CONSENT_IS_CHECKED'] = $arParams['USER_CONSENT_IS_CHECKED'];
		}

		// Флаг, что механизм согласия уже подгружен
		if (isset($arParams['USER_CONSENT_IS_LOADED']))
		{
			$componentElementParams['USER_CONSENT_IS_LOADED'] = $arParams['USER_CONSENT_IS_LOADED'];
		}

		$elementId = $APPLICATION->IncludeComponent('bitrix:catalog.element', 'jccatalog', $componentElementParams,
			$component
		);

		// Сохраняет ID текущего товара в глобальную переменную. Для других компонентов и кусков шаблона
		$GLOBALS['CATALOG_CURRENT_ELEMENT_ID'] = $elementId;

		if ($elementId > 0) // Компонент склад. Магазин А - 3 шт. Магазин Б - 10 шт., адрес, график и т.п.
		{ // Дополнительно тянет catalog, делает много запросов
